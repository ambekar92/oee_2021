...
userImpl.prototype.getUsers = function (query) {
  ...
  return new Promise((resolve, reject) => {
    User.find()
      .skip(query.skip)
      .limit(query.limit)
      .toArray(async function (findOneUserErr, findOneUserResult) {
        if (!findOneUserErr) {
...
function verifyLdap(user) {
  ...
  return new Promise((resolve, reject) => {
    ...
    var client = ldap.createClient({
      url: [
        "ldaps://us-ldap.nxp.com:636",
        "ldaps://eu-ldap.nxp.com:636",
        "ldaps://ap-ldap.nxp.com:636",
      ],
      tlsOptions: tlsOptions,
    });
    console.log("user--", user);
    client.bind(
      "cn=srv_wireless_automation, OU=SRV Accounts,OU=Accounts,OU=Service Delivery,DC=WBI,DC=NXP,DC=COM",
      "xA$ikRA35s57ppG7XQSl^#R&",
      function (err) {
        if (err) {
...
userImpl.prototype.disableUser = function (user) {
  ...
  return new Promise((resolve, reject) => {
    User.deleteOne(
      { nxfID: user.nxfID },
      function (insertUserErr, insertUserResult) {
        if (!insertUserErr) {
          resolve(insertUserResult);
        } else {
          reject(insertUserErr);
